...
Because `zgraph` graph structures are immutable (to preserve `jax.jit` compatibility), `thermograph` handles hot-swapping through **Envelope Replacement**. 
...
2. **Immutable Engines:** Do not attempt to modify the `self.engine` (which is a `zgraph` `eqx.Module`) in place. If the user changes a phase's physical model, you must fully re-compile and replace the `.engine` attribute.
3. **Indices to Buffers:** When building `zgraph` models from `thermograph`, ensure all indices are passed in such a way that the `zgraph` nodes can properly register them as buffers (e.g. passing lists of ints which `zgraph` nodes then cast to `eqx.field(static=True)`).
